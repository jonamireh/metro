...
=== "Anvil"
    ...
    * `@ContributesBinding` and `@ContributesIntoMap` use `priority` instead of Anvil's `rank`.
        * If Anvil interop is enabled, its numeric `rank` properties are treated as contribution priorities.
